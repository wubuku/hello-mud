...
Counts the number of lines of business logic code that we are involved in writing:

```shell
cd packages/contracts

find . \
    \( -path "./src/systems/*Logic.sol" -o -path "./src/utils/*.sol" \) \
    -type f | xargs cat | grep -v '^\s*$' | wc -l
```

The above statistics exclude blank lines and include comments. The signature part of the function that implements the business logic is actually generated by the low-code tool. 
And the code of function bodies, some of which is filled in by AI and some of which we modify manually.
