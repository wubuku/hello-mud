...
We copy the Move version of the code into the `XxxLogic.sol` files generated by the low-code tool, placing it within a comment block `/* Move version of the business logic implementation code */`. Then we let the AI generate the code first. We use the following prompt:
