...
2. The dddappp-mud tool converts DDDML models into:
   - MUD data model (`mud.config.ts`)
   - MUD Systems (business logic contracts)
3. The MUD toolchain further processes:
   - `tablegen` generates DAL code from the data model
   - `worldgen` extracts interface definitions from Systems
4. Finally, all these components combine into the complete contract code
...
- The System contract file for this model is in: `src/systems/ItemSystem.sol`
