...
//批量空投（使用Batchcall一个个添加） forge script BatchAirDropToIslands.s.sol:BatchCall --sig "run(address)" 0x63381030dda22c888f2548436c73146ef835ab9e --broadcast --rpc-url https://odyssey.storyrpc.io/
contract BatchCall is Script {
  ...
  uint32 resourceWoodCuttingQuantity = 88;
  uint32 resourceMinningQuantity = 88;
  uint32 resouceCottonSeedsQuantity = 88; //+112;
  ...
  function run(address worldAddress) external {
    ...
    // console.log("for($i = 0; $i -lt %d; $i++){", coordinatesX.length);
    // for (uint i = 0; i < coordinatesX.length; i++) {
    //   console.log(" $coordinate = [Coordinate]::new(%d, %d)", coordinatesX[i], coordinatesY[i]);
    //   console.log("$coordinates += $coordinate");
    // }
    // console.log("}");
    world.app__islandClaimWhitelistUpdate(address(0x76de4f3CBC645Df5089681e7A1D6F70816C8BA4b), false);
    world.app__mapAirdrop(uint32(2147483647), uint32(2147483647), resourcesItemIds, resourcesQuantities);
    world.app__islandClaimWhitelistUpdate(address(0x76de4f3CBC645Df5089681e7A1D6F70816C8BA4b), true);
    vm.stopBroadcast();
  }
