...
contract BugAboutBattleTest is Script {
  //
  // forge script BugAboutBattleTest.s.sol:BugAboutBattleTest --sig "run(address)" 0x593ad505023ea24371f8f628b251e0667308840f --broadcast --rpc-url https://odyssey.storyrpc.io/
  // forge script BugAboutBattleTest.s.sol:BugAboutBattleTest --sig "run(address)" 0x776086899eab4ee3953b7c037b2c0a13c7a1deed --broadcast --rpc-url https://odyssey.storyrpc.io/
  //
  function run(address worldAddress) external {
    ...
    for (uint x = 0; x < battleIds.length; x++) {
      ShipBattleData memory shipBattleData = ShipBattle.get(battleIds[x]);
      console.log("Battle Id:%d", battleIds[x]);
      ...
      RosterData memory rosterData = Roster.get(
      ...
      world.app__shipBattleTakeLoot(battleIds[x], 1);
    }
    vm.stopBroadcast();
  }
