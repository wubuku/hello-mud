...
contract RosterSystem is System, IAppSystemErrors {
  ...
  event EnvironmentRosterCreatedEvent(
    uint256 indexed playerId,
    uint32 indexed sequenceNumber,
    uint32 coordinatesX,
    uint32 coordinatesY,
  ...
  event RosterShipsPositionAdjustedEvent(
    uint256 indexed playerId,
    uint32 indexed sequenceNumber,
    uint64[] positions,
    uint256[] shipIds
  ...
  event RosterShipTransferredEvent(
    uint256 indexed playerId,
    uint32 indexed sequenceNumber,
    uint256 shipId,
  ...
  event RosterMultiShipsTransferredEvent(
    uint256 indexed playerId,
    uint32 indexed sequenceNumber,
    uint256[] shipIds,
    uint256 toRosterPlayerId,
  ...
  function rosterCreateEnvironmentRoster(
    uint256 playerId,
    ...
    if (
      !(rosterData.status == uint8(0) &&
        rosterData.speed == uint32(0) &&
        rosterData.baseExperience == uint32(0) &&
        rosterData.environmentOwned == false &&
        rosterData.updatedCoordinatesX == uint32(0) &&
        rosterData.updatedCoordinatesY == uint32(0) &&
        rosterData.coordinatesUpdatedAt == uint64(0) &&
        rosterData.targetCoordinatesX == uint32(0) &&
        rosterData.targetCoordinatesY == uint32(0) &&
        rosterData.originCoordinatesX == uint32(0) &&
        rosterData.originCoordinatesY == uint32(0) &&
        rosterData.sailDuration == uint64(0) &&
        rosterData.setSailAt == uint64(0) &&
        rosterData.currentSailSegment == uint16(0) &&
        rosterData.shipBattleId == uint256(0) &&
        rosterData.shipIds.length == 0)
    ) {
      revert RosterAlreadyExists(playerId, sequenceNumber);
    }
    EnvironmentRosterCreated memory environmentRosterCreated = RosterCreateEnvironmentRosterLogic.verify(
      playerId,
      sequenceNumber,
      coordinatesX,
      coordinatesY,
    ...
    emit EnvironmentRosterCreatedEvent(
      environmentRosterCreated.playerId,
      environmentRosterCreated.sequenceNumber,
      environmentRosterCreated.coordinatesX,
      environmentRosterCreated.coordinatesY,
      environmentRosterCreated.shipResourceQuantity,
      environmentRosterCreated.shipBaseResourceQuantity,
      environmentRosterCreated.baseExperience
    );
  ...
  function rosterAdjustShipsPosition(
    uint256 playerId,
    uint32 sequenceNumber,
    ...
    RosterShipsPositionAdjusted memory rosterShipsPositionAdjusted = RosterAdjustShipsPositionLogic.verify(
      playerId,
      sequenceNumber,
      positions,
      shipIds,
    ...
    emit RosterShipsPositionAdjustedEvent(
      rosterShipsPositionAdjusted.playerId,
      rosterShipsPositionAdjusted.sequenceNumber,
      rosterShipsPositionAdjusted.positions,
      rosterShipsPositionAdjusted.shipIds
    );
    RosterData memory updatedRosterData = RosterAdjustShipsPositionLogic.mutate(
  ...
  function rosterTransferShip(
    ...
    RosterShipTransferred memory rosterShipTransferred = RosterTransferShipLogic.verify(
      playerId,
      sequenceNumber,
      shipId,
      toRosterPlayerId,
    ...
    emit RosterShipTransferredEvent(
      rosterShipTransferred.playerId,
      rosterShipTransferred.sequenceNumber,
      rosterShipTransferred.shipId,
      rosterShipTransferred.toRosterPlayerId,
      rosterShipTransferred.toRosterSequenceNumber,
      rosterShipTransferred.toPosition,
      rosterShipTransferred.transferredAt
    );
    RosterData memory updatedRosterData = RosterTransferShipLogic.mutate(rosterShipTransferred, rosterData);
    Roster.set(playerId, sequenceNumber, updatedRosterData);
  }

  function rosterTransferMultiShips(
    uint256 playerId,
    uint32 sequenceNumber,
    uint256[] memory shipIds,
    uint256 toRosterPlayerId,
    ...
    RosterMultiShipsTransferred memory rosterMultiShipsTransferred = RosterTransferMultiShipsLogic.verify(
      playerId,
      sequenceNumber,
      shipIds,
      toRosterPlayerId,
    ...
    emit RosterMultiShipsTransferredEvent(
      rosterMultiShipsTransferred.playerId,
      rosterMultiShipsTransferred.sequenceNumber,
      ...
      rosterMultiShipsTransferred.toPosition,
