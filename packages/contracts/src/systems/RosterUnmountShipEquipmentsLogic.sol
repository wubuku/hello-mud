...
import { RosterShipEquipmentsUnmounted } from "./RosterEvents.sol";
...
/**
 * @title RosterUnmountShipEquipmentsLogic Library
 * @dev Implements the Roster.UnmountShipEquipments method.
 */
library RosterUnmountShipEquipmentsLogic {
  /**
   * @notice Verifies the Roster.UnmountShipEquipments command.
   * @param rosterData The current state the Roster.
   * @return A RosterShipEquipmentsUnmounted event struct.
   */
  function verify(
    uint256 playerId,
  ...
  ) internal pure returns (RosterShipEquipmentsUnmounted memory) {
    ...
    /*
    return RosterShipEquipmentsUnmounted({
  ...
  /**
   * @notice Performs the state mutation operation of Roster.UnmountShipEquipments method.
   * @dev This function is called after verification to update the Roster's state.
   * @param rosterShipEquipmentsUnmounted The RosterShipEquipmentsUnmounted event struct from the verify function.
   * @param rosterData The current state of the Roster.
   * @return The new state of the Roster.
   */
  function mutate(
    RosterShipEquipmentsUnmounted memory rosterShipEquipmentsUnmounted,
    RosterData memory rosterData
    ...
    //rosterData.{STATE_PROPERTY} = rosterShipEquipmentsUnmounted.{EVENT_PROPERTY};
    return rosterData;
  }
}
